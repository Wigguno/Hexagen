					...
					for n = 1, table.getn(NeighbourDiffs) do
						...
						then
							...
							if neighbour ~= nil and next(neighbour) ~= nil then
								hex["neighbours"][n] = neighbour["name"]
							end
						end
					end
	...
	for HexNum = 0, HexList["HexCount"] do
		...
		for NodeNum = 1,6 do
			...
			if HexData["nodes"][NodeNum] == nil then
				...
				NodeData["name"] = "Node_" .. NodeCount
				NodeData["location"] = HexData["location"] + offset
				NodeData["pathable"] = true
				NodeData["neighbours"] = {}
				HexList[NodeData["name"]] = NodeData

				HexData["nodes"][NodeNum] = NodeData["name"]
				...
				for NeighbourDirection, Neighbour in pairs(HexData["neighbours"]) do
					if NeighbourDirection == TargetNeighbour1 then
						...
						if HexList[Neighbour]["nodes"][Neighbour1NodeNum] == nil then
							HexList[Neighbour]["nodes"][Neighbour1NodeNum] = NodeData["name"]
						end
					...
					elseif NeighbourDirection == TargetNeighbour2 then
						...
						-- Check if this node has already been generated, and store it
						if HexList[Neighbour]["nodes"][Neighbour2NodeNum] == nil then
							HexList[Neighbour]["nodes"][Neighbour2NodeNum] = NodeData["name"]
						end
					end
				end
			end		
	...
	for HexNum = 0, HexList["HexCount"] do
		...
		for Num = 1,6 do
			...
			if HexData["neighbours"][NeighbourNum1] ~= nil then
				...
				local HexNeighbourData = HexList[HexNeighbourName]
